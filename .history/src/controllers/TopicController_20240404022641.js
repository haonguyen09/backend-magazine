...
const createTopic = async(req, res) => {
    try {
        ...
        const { title, description, startDate, closureDate, user_id, faculty_id} = req.body;
        
        if (!title || !description || !startDate || !closureDate || !user_id || !faculty_id) {
        ...
        if (start >= closure) {
            return res.status(400).json({
                status: 'ERR',
                message: 'Dates must be logically ordered: startDate < closureDate'
            });
        }
