...
const FilterTopicArticle = async (req, res) => {
    const {  limit, page, filterStatus, filterTopic, filterName } = req.query;
    try {
        const response = await ArticleService.FilterTopicArticle(limit, page, filterStatus, filterTopic, filterName);
        return res.status(200).json(response);
    } catch (error) {
