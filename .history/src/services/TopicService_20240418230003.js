...
const getPaginationTopic = (limit = 4, page = 0, filterFaculty, filterTitle) => {
    return new Promise(async (resolve, reject) => {
        try {
            const queryConditions = {};
            if (filterFaculty) {
                // Assuming filterFaculty is the ID of the faculty
                queryConditions.faculty_id = mongoose.Types.ObjectId(filterFaculty);
            }
            if (filterTitle) {
                queryConditions.title = { '$regex': filterTitle, '$options': 'i' };
            }

            const totalTopics = await Topic.countDocuments(queryConditions);
            const topics = await Topic.find(queryConditions)
                .limit(limit)
                .skip(page * limit);

            // Assuming you may want to know how many topics meet some condition of being 'approved'
            // You'll need to adjust this to match your schema (e.g., if you have an 'isApproved' field)
            const totalApproved = await Topic.countDocuments({ ...queryConditions, isApproved: true });
            ...
            resolve({
                ...
                totalPage: Math.ceil(totalTopics / limit),
            });
        } catch (e) {
