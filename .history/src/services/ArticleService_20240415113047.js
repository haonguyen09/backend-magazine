...
const PaginationArticle = (limit = 4, page = 0, filterStatus, filterName) => {
    return new Promise(async (resolve, reject) => {
        try {
            const totalArticle = await Article.countDocuments()
            const totalApproved = await ((await Article.find()).filter(item => item.status === "approved"))
            // console.log("totalApproved", totalApproved.length)
            const allArticle = await Article.find().limit(limit).skip(page * limit)

            if (filterStatus) {
                const objectFilterStatus = {}
                objectFilterStatus[filterStatus[0]] = filterStatus[1]
                const labelStatus = filterStatus[0]
                const allObjectFilterStatus = await Article.find({
                    [labelStatus] : filterStatus[1]
                }).limit(limit).skip(page * limit)

                resolve({
                    status: 'OK',
                    message: 'Success',
                    data: allObjectFilterStatus,
                    total: totalArticle,
                    totalApproved: totalApproved.length,
                    pageCurrent: Number(page) + 1,
                    totalPageApproved: Math.ceil((totalApproved.length) / limit),
                    totalPage: Math.ceil(totalArticle / limit),
                })
            }

            if (filterName) {
                const objectFilterName = {}
                objectFilterName[filterName[0]] = filterName[1]
                const labelName = filterName[0]
                const allObjectFilterName = await Article.find({
                    [labelName]: {'$regex': filterName[1] }
                }).limit(limit).skip(page * limit)
                
                resolve({
                    status: 'OK',
                    message: 'Success',
                    data: allObjectFilterName,
                    total: totalArticle,
                    totalApproved: totalApproved.length,
                    pageCurrent: Number(page) + 1,
                    totalPageApproved: Math.ceil((totalApproved.length) / limit),
