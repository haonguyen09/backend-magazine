...
const createArticle = async (newArticle, files) => {
    ...
    try {
        ...
        // Create the article data with relative paths for document and images
        const articleData = {
            title,
            content,
            user_id: new mongoose.Types.ObjectId(user_id),
            ...
            ratingsCount,
            topic_id: new mongoose.Types.ObjectId(topic_id),
            images: imagePaths, // Store only the image filenames
        ...
        // Create the article without images/comments initially
        const createdArticle = await Article.create(articleData);
        const articleId = createdArticle._id; // <--- Move this line after creating the article
