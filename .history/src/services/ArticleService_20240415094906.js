...
const PaginationArticle = (limit = 4, page = 0, filterStatus, filterName) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const totalApproved = await Article.find((item) => item.status === "approved")
            ...
            if (filterStatus) {
                ...
                const allObjectFilterStatusP = await Article.find({
                ...
                resolve({
                    status: 'OK',
                    message: 'Success',
                    data: allObjectFilterStatusP,
                    totalApproved: allObjectFilterStatus,
                    total: totalArticle,
