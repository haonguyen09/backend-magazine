...
const createArticle = async (newArticle, files) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // Store relative paths for images and documents
            let documentPath = 'uploads/default/document'; // Relative path for default document
            let imagePath = 'uploads/default/image'; // Relative path for default image

            if (files) {
                // Handle document upload
                if (files.document && files.document[0]) {
                    documentPath = files.document[0].path.replace('src', ''); // Modify the path to be relative
                }

                // Handle image uploads
                if (files.image && files.image[0]) {
                    imagePath = files.image[0].path.replace('src', ''); // Modify the path to be relative
                }
            }
            ...
            const createdArticle = await Article.create({
                ...
                ratingsCount,
                topic_id: new mongoose.Types.ObjectId(topic_id),
                images: [{ file_path: imagePath }], // Store the relative path for images
                document: documentPath, // Store the relative path for document
                docHtmlContent
            });
