...
const getPaginationTopic = (limit = 4, page = 0, filterFaculty, filterTitle) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const totalTopic = await Topic.countDocuments(queryConditions);
            const allTopic = await Topic.find(queryConditions).limit(limit).skip(page * limit);

            // Calculation for approved Topics
            const totalTopicByFaculty = await Topic.countDocuments({ status: 'approved' });
            const totalPageApproved = Math.ceil(totalTopicByFaculty / limit);
