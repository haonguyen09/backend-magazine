...
const createArticle = async (newArticle, files) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (files.document && files.document[0]) {
                documentPath = path.join(__dirname, '..', 'uploads', files.document[0].filename);
            }

            if (files.image && files.image[0]) {
                imagePath = path.join(__dirname, '..', 'uploads', files.image[0].filename);
            }
            ...
            if (files.image) {
                for (const file of files.image) {
                    const newImage = await Image.create({ file_path: path.join(__dirname, '..', 'uploads', file.filename), article_id: articleId });
                    imageIds.push(newImage._id);
                }
            }
