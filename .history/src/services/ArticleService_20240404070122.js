    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (files['document'] && files['document'][0]) {
                newArticleData.documentPath = files['document'][0].path; // Adjust field as per your schema
            }
            ...
            if (files['image']) {
                for (const file of files['image']) {
                    ...
                    const newImage = await Image.create({ file_path: imagePath, article_id: /* You'll need the Article ID here, consider creating the Article first */ });
                    imageIds.push(newImage._id);
                }
                newArticleData.images = imageIds;
            }
