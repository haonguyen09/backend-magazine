...
const { sendEmailMarking, sendEmailCUArticle, sendEmailReportReview } = require('../services/EmailService');
...
const routes = (app) => {
    ...
    app.post("/api/sendEmailReportReview", (req, res) => {
        const { email, title, name, emailUser, postDate, DateRemaining } = req.body;
        sendEmailReportReview(email, title, name, emailUser, postDate, DateRemaining)
            .then(() => {
                res.status(200).json({ success: true, message: "Email sent successfully" });
            })
            .catch((error) => {
